...
const TIMEOUT: u64 = 2 * 60 * 100; //3 minutes
...
fn forward(bind_addr: &str, local_port: i32, remote_host: &str, remote_port: i32) {
    ...
    let local = UdpSocket::bind(&local_addr).expect(&format!(
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if responder.send_to(to_send, dest).is_err() {
                eprintln!(
    ...
    let fake_addr: (usize, SocketAddr) = (
        0,
        SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(0, 0, 0, 0), 0)),
    );

    loop {
        let (num_bytes, src_addr) = local.recv_from(&mut buf).unwrap_or(fake_addr);
        ...
        loop {
            ...
            let sender = client_map.entry(client_id.clone()).or_insert_with(|| {
                ...
                thread::spawn(move|| {
                    ...
                    println!("client {} -> {}", src_addr, &temp_outgoing_addr);
                    let upstream_send = if let Ok(send) = UdpSocket::bind(&temp_outgoing_addr) {
                        send
                    } else {
                        return eprintln!("Failed to bind to transient address {}", &temp_outgoing_addr)
                    };
                    let upstream_recv = if let Ok(recv) = upstream_send.try_clone() {
                        recv
                    } else {
                        return eprintln!("Failed to clone client-specific connection to upstream!");
                    };
                        ...
                        loop {
                            match upstream_recv.recv_from(&mut from_upstream) {
                                Ok((bytes_rcvd, _)) => {
                                    ...
                                    if local_send_queue.send((src_addr, to_send)).is_err() {
                                        eprintln!("Failed to queue response from upstream server for forwarding!")
                                    }
                                },
                                Err(_) => {
                                    if local_timed_out.load(Ordering::Relaxed) {
                                        format!("client {} -> closed", src_addr);
                                        break;
                                    }
                                }
                            };
                    ...
                    loop {
                        match receiver.recv_timeout(Duration::from_millis(TIMEOUT)) {
                            Ok(from_client) =>  {
                                if upstream_send.send_to(from_client.as_slice(), &remote_addr_copy).is_err() {
                                    eprintln!("Failed to forward packet from client {} to upstream server!", src_addr);
                                } else {
